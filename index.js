...
const Shares = {
    ...
    async createLinkShare({ path, permissions = 'read', password = null, expireDate = null }) {
        ...
        // Path relativ zum User im Files-Bereich aufbauen
        const cleanPath = path.startsWith('/') ? path.slice(1) : path;
        const filesPath = `/${cleanPath}`; // wir nutzen denselben Namespace wie Files.list

        const url = `${CONFIG.url.replace(/\/+$/, '')}/ocs/v2.php/apps/files_sharing/api/v1/shares`;

        const permMap = {
            read: 1,
            edit: 15 // Nextcloud-Standard: create+update+delete+share, bei Bedarf feiner aufsplitten
        };
